...
def fast_cd_text(batch, model, start, stop):
    # this "batch" actually gets changed by the acd code....ha
    # not always the fixed
    ...
    W_x = weights['weight_ih_l0']  # 4 * h_dim x word_dim (512 x 300)
    W_h = weights['weight_hh_l0']  # 4 * h_dim x h_dim (512 x 128)
    ...
    for i in range(T):
        ...
        rel_gates = np.dot(W_h, prev_rel_h)
        irrel_gates = np.dot(W_h, prev_irrel_h)
        ...
        if i >= start and i <= stop:
            ...
            rel_gates += np.dot(W_x, word_vecs[i])
            ...
            irrel_gates += np.dot(W_x, word_vecs[i])

        rel_i, rel_g, rel_f, rel_o = np.split(rel_gates, 4, 0)
        irrel_i, irrel_g, irrel_f, irrel_o = np.split(irrel_gates, 4, 0)
...
def snli_model_process(batch, model, start, stop, premise=True):
    # batch = x / input in your code!!!

    input_premise, length_premise = batch.premise
    input_hypothesis, length_hypothesis = batch.hypothesis

    embed_premise = model.embed(input_premise)
    embed_hypothesis = model.embed(input_hypothesis)
    output_premise = model.autolen_rnn(embed_premise, length_premise)
    output_hypothesis = model.autolen_rnn(embed_hypothesis, length_hypothesis)

    # we get relevant vector from LSTM
    # for SNLI
    ...
    if premise:
        rel_premise = snli_cd_text(embed_premise, model, start, stop)
        rel_concat = torch.cat([rel_premise, output_hypothesis,
                                rel_premise - output_hypothesis,
                                rel_premise * output_hypothesis], dim=1)
        # scores for each label
        # TODO: for Yuhui, fix the dimension!!!
        scores = np.dot(np.dot(fc1_W, rel_concat) + fc1_b, out_W) + out_b
    else:
        rel_hyp = snli_cd_text(embed_hypothesis, model, start, stop)
        rel_concat = torch.cat([output_premise, rel_hyp,
                                output_premise - rel_hyp,
                                output_premise * rel_hyp], dim=1)
        # scores for each label
        # TODO: for Yuhui, fix the dimension!!!
        scores = np.dot(np.dot(fc1_W, rel_concat) + fc1_b, out_W) + out_b
...
def snli_cd_text(sentence, model, start, stop):
